...
-- shuffle elems such that each thread uniformly chooses slot
--
shuffleM' :: [a] -> IO [a]
shuffleM' [] = return []
shuffleM' xs = shuffleM xs

threadUpdates :: Int -> Int -> Int -> IO [(Int, Int)]
threadUpdates numThreads numUpdates threadId = do
    shuffleM $ zip elems (tail elems)
  where
    elems = [ i * (numThreads + 1) + threadId + 1
...
initialElems :: Int -> Int -> [Int]
initialElems numThreads numInitialElems =
    [ i * (numThreads + 1) | i <- [1..numInitialElems] ]
...
runUpdates
    ...
    -> IO ()
runUpdates
    name newState insert0 delete0 numThreads numStripes numUpdates numInitialElems
  = do
    putStr "- "
    vars <- V.fromList <$> replicateM numStripes newState

    let insert i = insert0 (vars V.! (i `mod` numStripes)) i
        delete i = delete0 (vars V.! (i `mod` numStripes)) i

    -- populate initial state
    mapM_ insert (initialElems numThreads numInitialElems)

    -- create jobs
    jobs <- forM [0..numThreads] $ \threadIdx -> do
        updates <- threadUpdates numThreads numUpdates threadIdx
        return $ do
            forM_ updates $ \(i, i') -> do
                -- We undo the previous update to keep the state sizes
...
data Ref a = forall r. Ref String (a -> IO r) (r -> (a -> a) -> IO ())

ioRef :: Ref a
ioRef = Ref "IORef" newIORef atomicModifyIORef'_

ioRefCas :: Ref a
ioRefCas = Ref "IORef CAS" newIORef atomicModifyIORefCAS_

tVar :: Ref a
tVar = Ref "TVar" newTVarIO atomicModifyTVar'_

tVarLazy :: Ref a
tVarLazy = Ref "TVar Lazy" newTVarIO atomicModifyTVar_

mVar :: Ref a
mVar = Ref "MVar" newMVar atomicModifyMVar'_
